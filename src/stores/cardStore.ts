...
/** LLM analysis endpoint — your own service, unchanged. */
const LLM_ANALYZE_URL = 'http://localhost:5000/analyze'
...
export const useCardStore = defineStore('cardStore', () => {
    ...
    async function fillQueue(deckName?: string): Promise<void> {
        ...
        try {
            ...
            if (rawCards[0]) {
                ...
                if (firstKey) {
                    console.debug(
                        '[fillQueue] first field value:',
                        rawCards[0].fields[firstKey]?.value?.slice(0, 200),
                    )
                }
            }
     ...
     */
    async function submitReview(markdownResponse: string): Promise<void> {
        ...
        try {
            const res = await fetch(LLM_ANALYZE_URL, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    card_id: cardToAnalyze.cardId,
                    question: cardToAnalyze.question,
                    correct_answer: cardToAnalyze.answer,
                    user_response: markdownResponse,
                }),
            })
            ...
            if (entry) {
                ...
                entry.llmAnalysis = data.analysis
            }
        } catch (err) {
